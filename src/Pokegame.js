...
const Pokegame = () => {
  ...
  // Iterate through the Pokemon array, assign randomly to either firstDeck or secondDeck and 
  // add that Pokemon item's base_experience to its respective deck
  Pokemon.forEach(value => {
    if ((Math.random() > 0.5 && firstDeck.length < 4) || secondDeck.length >= 4) {
      firstDeck.push(value);
      firstTotal += value.base_experience;
    } else {
      secondDeck.push(value);
      secondTotal += value.base_experience;
    }
  });
